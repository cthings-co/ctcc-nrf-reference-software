...
* **`--filename-max-length=128`.** The committed names are `format-patch` output at
  ...
  `0002-boards-ct-ctcc-add-nRF9151-target-and-adopt-DK-dongle-improvements.patch`,
  comes out as `0002-boards-ct-ctcc-add-nRF9151-target-and-adopt-DK-dongl.patch` - so
...
* `DCO sign-off` and `VERSION bump` - on every pull request, on GitHub-hosted
  runners.
* `Build <target>` - the firmware/bootloader/assembly-test matrix. It runs only
  after the required pull-request checks pass and is not run for fork pull requests.
  If a required check is red, fix it and push again before requesting build output.

Questions about firmware deployment, signing keys, or hardware: support@cthings.co
