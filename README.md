...
sign an image it accepts, so it is meant for development. The reference loader image is
not authenticated by another component in the configurations documented here; it is
programmed directly as part of the reference setup. This repository points both the
loader's verification key and the application's
...
For assistance with firmware deployment or hardware, please reach out:
support@cthings.co
...
> [!WARNING]
> The reference configuration used by these commands deliberately keeps debug
> access enabled. Do not remove or override its APPROTECT settings unless you
> intend to lock SWD: an APPROTECT UICR write may require `nrfjprog --recover`,
> which erases the device. See [Software considerations](#software-considerations)
...
overrides it, so this is a stock SDK build. `0x00000` is not unused: the full-flash
`merged.hex` places MCUboot there. The signed application image begins at the
`image-0` address shown below. The two SoCs do not share a map, so both are given
here.
...
> **Use images only with the compatible bootloader and flash layout supplied by this repository.**
>
> **Do not use MCUboot serial recovery to move between this repository's `main`
> reference artifacts built with NCS v3.1.0 and this branch's reference artifacts
> built with NCS v3.4.0, in either direction.** The image-slot boundaries differ,
> and serial recovery cannot translate them; the installed loader would write an
> image at addresses that do not match the image's layout. A loader must also verify
> the signing key used for its application image.
>
> To cross that compatibility boundary, establish a known reference configuration
> by flashing the full ``merged.hex`` from this repository over SWD.
...
Other deployments may want to deviate from all of this - a single-slot loader saves
...
`tests/assembly/` is a hardware qualification test image: a ztest application
...
* On **nRF9151 / nRF9161** the flashable file is `build/zephyr/tfm_merged.hex`
  ...
  does not boot. The `/ns` overlay relocates the slot0 group to flash origin, so
  `tfm_merged.hex` places TF-M at `0x0` and **overwrites the Open Bootloader**. It
  is a standalone test image, not an MCUboot-signed update. **Erase it first**:
  ...
  Reflash the Open Bootloader afterwards to restore the standard serial-recovery
  and update path - see [Building the Open Bootloader on its own](#building-the-open-bootloader-on-its-own).
* It deliberately does **not** lock the debug port (`tests/assembly/prj.conf`), so
  ...
  `nrfjprog --recover`. So does every other image in this repository - the reference configurations in this repository leave debug access enabled; see
...
* **The reference configurations selected by this repository leave debug access enabled**,
  so a card stays reprogrammable over
  ...
  *only* actor there, because the helper is excluded from a non-secure image and the
  ...
  `tests/assembly/boards/ctcc_nrf91*_ns.conf`. That is deliberate for a development
  reference signed with MCUboot's public key - locking would protect nothing while making the card
  awkward to work with. Assess and configure debug protection separately for any
  deployed firmware.
* A bootloader not built by this repository may use a different reset mechanism after a
  serial update; follow that bootloader's documentation.
